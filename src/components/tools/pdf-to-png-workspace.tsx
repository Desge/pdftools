// ─── PDF to PNG Workspace ───
// Interactive PDF-to-PNG converter with resolution scale options.
// Uses PDF.js to render each page to Canvas → exports as PNG → bundles into ZIP.
"use client";
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : "An error occurred while converting the PDF to PNG."
      );
      setProgress(null);
    } finally {
      setProcessing(false);
    ...
    <div>
      ...
      {result && !processing && (
        <div className="rounded-2xl border-2 border-green-200 bg-green-50/50 p-6 text-center dark:border-green-800 dark:bg-green-950/20">
          ...
          <p className="mb-4 text-sm text-green-700 dark:text-green-500">
            {result.totalPages} pages converted → PNG · ZIP downloaded
          </p>
      ...
      {!file && !processing && !result && !error && (
        <p className="mt-3 text-center text-xs text-gray-400">
          Upload a PDF to convert each page to a PNG image
        </p>
      )}
    </div>
  );
