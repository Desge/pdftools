...
// Converts HTML files to PDF using DOMPurify→html2pdf.js
// Options: paper size (A4/Letter), margins (narrow/medium/wide), scale (1x/1.5x/2x)
"use client";
...
export function HtmlToPdfWorkspace() {
  ...
  const handleProcess = useCallback(async () => {
    if (!file) return;
    setProcessing(true);
    setProgress(dict.workspace.loadingHtml);
    setError(null);
    setResult(null);
    ...
    try {
      ...
      setResult({
        message: `Converted "${file.name}" to PDF (${PAPER_SIZES[paperSize].format.toUpperCase()}, ${SCALE_LABELS[scale]}).`,
  ...
  }, [file, paperSize, margin, scale, dict]);
      ...
      {file && !processing && !result && !error && (
        <div className="space-y-4">
          ...
          <div className="rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-800 dark:bg-gray-900">
            ...
            <div>
              <label className="mb-1.5 block text-xs font-medium text-gray-600 dark:text-gray-400">
                {dict.workspace.renderScale}
              </label>
              <div className="flex gap-2">
                {([1, 1.5, 2] as Scale[]).map((s) => (
                  <button
                    key={s}
                    onClick={() => setScale(s)}
                    className={`flex-1 rounded-lg px-3 py-2 text-center text-xs font-semibold transition-all ${
