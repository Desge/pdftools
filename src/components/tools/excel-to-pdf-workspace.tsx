...
// Converts XLSX files to PDF using SheetJS→DOMPurify→html2pdf.js
// Options: orientation (auto/portrait/landscape), margins (narrow/medium/wide)
"use client";
...
export function ExcelToPdfWorkspace() {
  ...
  const handleProcess = useCallback(async () => {
    if (!file) return;
    setProcessing(true);
    setProgress(dict.workspace.parsingSpreadsheet);
    setError(null);
    setResult(null);
    ...
    try {
      ...
      let jsOrientation: "portrait" | "landscape";
      if (orientation === "auto") {
        // Auto-detect: if sheet has many columns, use landscape
        const cols = sheet["!ref"]
          ? XLSX.utils.decode_range(sheet["!ref"]).e.c + 1
          : 1;
        jsOrientation = cols > 6 ? "landscape" : "portrait";
      } else {
        jsOrientation = orientation;
      }
      ...
      const pdfBlob: Blob = await html2pdf
        ...
        .set({
          ...
          html2canvas: { scale: 2 },
          jsPDF: { orientation: jsOrientation, unit: "mm", format: "a4" },
        })
        .outputPdf("blob");
      ...
      setResult({
        message: `Converted "${sheetName}" from "${file.name}" to PDF (${jsOrientation}, ${margin} margin).`,
            ...
            <div className="mb-4">
              ...
              <div className="flex gap-2">
                {(["auto", "portrait", "landscape"] as Orientation[]).map((o) => (
                  ...
                  >
                    {o === "auto" ? dict.workspace.auto : o === "portrait" ? dict.workspace.portrait : dict.workspace.landscape}
                  </button>
                ))}
              </div>
            </div>
